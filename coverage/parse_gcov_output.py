...
def display_file_coverage(per_file_coverage, total_coverage):
    ...
    header_template = "%-s\t%10s\t\t" + "%-" + str(max_file_name_length) + "s"
    separator = "-" * (max_file_name_length + 10 + 15)
    print(
        header_template % ("Coverage", "Lines", "Filename")
    )  # noqa: E999 T25377293 Grandfathered in
    print(separator)

    # -- Print body
    # template for printing coverage report for each file.
    record_template = "%-5.2f%%\t%10d\t\t\t" + "%-" + str(max_file_name_length) + "s"

    per_file_coverage = dict(sorted(per_file_coverage.items()))
    for fname, coverage_info in per_file_coverage.items():
        coverage, lines = coverage_info
        print(record_template % (coverage, lines, fname))

    # -- Print footer
    if total_coverage:
        print(separator)
        print(record_template % (total_coverage[0], total_coverage[1], "Total"))
